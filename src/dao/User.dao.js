...
export default class UserDAO{
    ...
    addProductToCart = async(email,pid,quantity)=>{
        const exists = await this.checkProduct(email,pid)
        if(exists===true){
            const prodInCart = await userModel.updateOne({email:email, "cart.productId":pid},{$inc:{"cart.$.quantity":quantity}})
            return prodInCart
        }else{
            const prodInCart = await userModel.updateOne({email:email},{$push:{cart:{productId:pid,quantity:quantity}}})
            return prodInCart
        }
    }
